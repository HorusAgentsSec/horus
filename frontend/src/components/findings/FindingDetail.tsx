...
interface Finding {
  ...
  raw_data: {
    ...
    debate?: { red?: string; blue?: string; red_rebuttal?: string; blue_rebuttal?: string } | null
    ssvc?: {
      priority?: string
      label?: string
      rationale?: string
  ...
  return (
    <div className="space-y-6">
      <div className="bg-surface border border-border rounded-lg p-6">
        ...
        {finding.raw_data.reappeared_at && (
          <div className="mt-3 flex items-center gap-1.5 text-xs text-severity-critical bg-severity-critical/10 border border-severity-critical/30 rounded px-2 py-1.5 w-fit">
            <AlertTriangle className="w-3.5 h-3.5" />
            Regression: this was marked resolved but this scan found it again.
          </div>
        )}
        ...
        {finding.raw_data.debate && (finding.raw_data.debate.red || finding.raw_data.debate.blue) && (
          <div className="mt-4 p-3 bg-bg rounded border border-border">
            ...
            {(finding.raw_data.debate.red_rebuttal || finding.raw_data.debate.blue_rebuttal) && (
              <div className="mt-3 pt-3 border-t border-border">
                <p className="text-[10px] font-medium text-muted uppercase mb-2">
                  Round two — critical &amp; internet-facing, so each side had to rebut the other
                </p>
                <div className="grid sm:grid-cols-2 gap-3">
                  <div className="p-2 rounded border border-severity-critical/20 bg-severity-critical/[0.04]">
                    <p className="text-[10px] font-semibold uppercase tracking-wide text-severity-critical mb-1">Red — rebuttal</p>
                    <p className="text-xs text-white/80 leading-relaxed">{finding.raw_data.debate.red_rebuttal || '—'}</p>
                  </div>
                  <div className="p-2 rounded border border-mode-auto/20 bg-mode-auto/[0.04]">
                    <p className="text-[10px] font-semibold uppercase tracking-wide text-mode-auto mb-1">Blue — rebuttal</p>
                    <p className="text-xs text-white/80 leading-relaxed">{finding.raw_data.debate.blue_rebuttal || '—'}</p>
                  </div>
                </div>
              </div>
            )}
