...
import { UserPlus, Trash2, Eye, Wrench, Crown } from 'lucide-react'
                    ...
                    <td className="py-3 px-4">
                      {can('admin') ? (
                        <select
                          ...
                          className="bg-bg border border-border rounded px-2 py-1 text-xs focus:outline-none focus:border-accent"
                        >
                          {(Object.keys(ROLE_META) as Role[]).map((r) => (
                            <option key={r} value={r}>{ROLE_META[r].label}</option>
                          ))}
