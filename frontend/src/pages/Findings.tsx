...
// Two findings are "the same vuln" if they share a CVE (the same CVE across N assets collapses
// even when the analyst LLM gave them slightly different titles); with no CVE, fall back to the
// normalized title.
function vulnKey(f: Finding): string {
  const cves = (f.cve_ids ?? []).filter(Boolean)
  if (cves.length) return 'cve:' + [...cves].sort().join(',')
  return 'title:' + f.title.trim().toLowerCase().replace(/\s+/g, ' ')
}

// Label for a collapsed group: the title of its most severe finding (titles may differ within it).
function groupLabel(items: Finding[]): string {
  return items.reduce((w, f) => ((SEV_RANK[f.severity] ?? 0) > (SEV_RANK[w.severity] ?? 0) ? f : w), items[0]).title
}
...
export default function Findings() {
  ...
  // findings (e.g. nmap vulners) are grouped by vuln so the same vulnerability across
  // several assets collapses into one row instead of N near-identical duplicates.
  const groups: Record<string, Finding[]> = {}
  const byVuln = new Map<string, Finding[]>()
  for (const f of findings) {
    const svc = f.raw_data?.source_service
    if (svc) (groups[svc] ??= []).push(f)
    else {
      const key = vulnKey(f)
      byVuln.set(key, [...(byVuln.get(key) ?? []), f])
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <Select
          ...
          options={SEVERITIES.map((s) => ({ value: s, label: s || 'All severities' }))}
        />
        <Select
          value={status}
        ...
        <Select
          value={tool}
          onValueChange={(v) => setTool(v)}
          options={TOOLS.map((t) => ({ value: t, label: t || 'All tools' }))}
        />
        <Select
          value={orderBy}
      ...
      {loading ? (
        ...
        <div className="stagger space-y-2">
          {[...byVuln.entries()].map(([key, items]) =>
            items.length === 1 ? (
              <FindingCard
                key={items[0].id}
                finding={items[0]}
                selected={selecting ? selectedIds.has(items[0].id) : undefined}
                onToggle={selecting ? () => toggleSelect(items[0].id) : undefined}
              />
            ) : (
              <VulnGroup
                key={key}
                title={groupLabel(items)}
                items={items}
                selecting={selecting}
...
// The same vulnerability (same title/template) detected on several assets: one collapsible
// row instead of N near-identical duplicates. The finding rows themselves stay per-asset.
function VulnGroup({ title, items, selecting, selectedIds, onToggle }: VulnGroupProps) {
  const [open, setOpen] = useState(false)
  const assetCount = new Set(items.map((f) => f.assets?.host ?? f.id)).size
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-xs text-muted shrink-0">
            {assetCount} asset{assetCount === 1 ? '' : 's'} affected
          </span>
        </div>
