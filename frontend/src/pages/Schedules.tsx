...
function ScanScheduleForm({ initial, onSaved, onCancel }: { initial?: ScanSchedule; onSaved: () => void; onCancel: () => void }) {
  ...
  return (
    <div className="bg-surface border border-border rounded-b-lg border-t-0 px-4 pb-4 pt-3 space-y-4">
      <p className="text-xs text-muted font-medium uppercase tracking-wide">{isEdit ? `Editing "${initial.name}"` : 'New scan schedule'}</p>
      <div>
...
function PhishingScheduleForm({
  ...
  return (
    <div className="bg-surface border border-border rounded-b-lg border-t-0 px-4 pb-4 pt-3 space-y-4">
      <p className="text-xs text-muted font-medium uppercase tracking-wide">{isEdit ? `Editing "${initial.name}"` : 'New phishing schedule'}</p>
      <div>
        <label className={labelCls}>Name</label>
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <div>
        ...
        <select className={inputCls} value={preset} onChange={(e) => onPreset(e.target.value)}>
