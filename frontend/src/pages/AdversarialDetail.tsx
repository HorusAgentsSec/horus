...
const CATEGORY_STYLES: Record<string, string> = {
  ...
  attack_chain:  'bg-horus-carnelian/10 text-horus-carnelian border-horus-carnelian/30',
...
export default function AdversarialDetail() {
  ...
  const deleteFinding = async () => {
    if (!id) return
    if (!(await confirm({ message: 'Delete this finding permanently? This cannot be undone.', danger: true, confirmLabel: 'Delete' }))) return
    setSaving(true)
    try {
