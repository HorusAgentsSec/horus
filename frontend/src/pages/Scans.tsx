...
export default function Scans() {
  ...
  const scanAll = async () => {
    if (!(await confirm(`Queue a scan for all ${activeAssetCount} active asset${activeAssetCount === 1 ? '' : 's'}?`))) return
    setScanningAll(true)
    try {
  ...
  const cancelActive = async () => {
    if (!(await confirm({ message: `Cancel all ${activeCount} active scan${activeCount === 1 ? '' : 's'}?`, danger: true, confirmLabel: 'Cancel scans' }))) return
    setCanceling('active')
    try {
