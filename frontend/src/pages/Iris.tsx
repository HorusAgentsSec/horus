...
import { useConfirm, useAlert } from '../components/ui/ConfirmProvider'
...
export default function Iris() {
  ...
  const saveTriageInterval = async (minutes: number) => {
    ...
    try {
      ...
      await alert(friendlyErrorMessage(e, 'Failed to save interval'))
    } finally {
  ...
  const deleteAgent = async (agent: IrisAgent) => {
    ...
    if (!(await confirm({
      message: `Delete agent "${agent.name}" from the dashboard?\n\n` +
    ...
    try {
      ...
      await alert(friendlyErrorMessage(e, 'Failed to delete agent'))
    }
  }
  ...
  const analyzeAgent = async (agent: IrisAgent) => {
    try {
      ...
      await alert(`Analysis triggered for "${agent.name}"`)
    } catch (e) {
      await alert(friendlyErrorMessage(e, 'Failed to trigger analysis'))
    }
  }
