"""
RiskManagerAgent — assigns execution mode (auto/approval_required/suggest_only)
...
wins); anything they don't cover falls back to the SSVC deployer assessment (core.ssvc) — the
contextual priority of the underlying finding (exploitation × exposure × automatable × impact).
...
def resolve_mode(
    ...
    """
    Returns (mode, reason). Checks rules in order; first match wins.
    ...
    for rule in rules:
        ...
        if "severity_max" in conditions:
            max_sev = conditions["severity_max"]
            # We don't have per-suggestion severity here — skip condition if not applicable
            # (severity filtering is better applied upstream)
            pass
...
class RiskManagerAgent(BaseAgent):
    ...
    def run(self, state: ScanState) -> ScanState:
        ...
        # Index the finding signals SSVC needs, keyed by finding fingerprint.
        ...
        for suggestion in state.remediation_suggestions:
            ...
            ssvc_result = None
            if not mode:
                # No explicit rule: fall back to the SSVC deployer assessment of the finding.
                ssvc_result = self._assess_ssvc(suggestion.finding_id, analyzed, enriched, state.asset)
        ...
        logger.info(
            "RiskManagerAgent: %d decisions (%d via SSVC, %d capped by safety, 0 tokens)",
