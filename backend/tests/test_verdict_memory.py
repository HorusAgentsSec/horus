...
def test_community_prior_applies_when_no_org_memory(monkeypatch):
    # A community false_positive prior only auto-applies for a non-risky finding (internal
    # asset, no elevated exploitability) — see the "risky" degrade-to-needs_verification guard
    # in ValidationAgent.run. _state() defaults to an internet-facing asset, so build one that
    # is internal here to exercise the auto-apply path this test is meant to cover.
    ...
    f = _finding("1")
    state = _state([f], [EnrichedFinding(finding_id="1", threat_context="", exploitability="none",
                                          public_exploits_exist=False)])
    state.asset.is_internal = True
    agent.run(state)
...
def test_community_false_positive_degrades_to_verification_when_internet_facing(monkeypatch):
    # Same community signal, but on an internet-facing asset (the _state() default): too risky
    # to silently suppress, so it degrades to needs_verification instead of auto-applying.
    _no_org_memory(monkeypatch)
    monkeypatch.setattr(verdict_memory, "recall_community", lambda sigs, client=None: {"svc:nginx": "false_positive"})
    agent = ValidationAgent()
    f = _finding("1b")
    agent.run(_state([f], [EnrichedFinding(finding_id="1b", threat_context="", exploitability="none",
                                           public_exploits_exist=False)]))

    assert f.verdict == "needs_verification"
