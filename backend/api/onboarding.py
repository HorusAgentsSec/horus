...
async def bootstrap(
    ...
    # Paid SaaS: orgs are provisioned by Horus (admin panel / payment), never self-served.
    # Only open-source single-tenant deployments flip this on.
    if not settings.allow_self_onboarding:
        raise HTTPException(
            status_code=403,
            detail="Self-service onboarding is disabled. Contact Horus to provision your organization.",
        )
