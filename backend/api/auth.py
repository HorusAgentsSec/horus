...
def _is_org_suspended(org_settings: Optional[dict]) -> bool:
    """True when a paying org has lapsed past the grace period and must be locked out.

    Only orgs with a Stripe subscription are subject to this — manual/enterprise/demo
    orgs (no stripe_subscription_id) are never suspended. Access stays normal while
    active/trialing/past_due (Stripe is still retrying); the grace window is measured from
    when the subscription finally went canceled/unpaid.
    """
    if not org_settings or not org_settings.get("stripe_subscription_id"):
        return False
    status_ = org_settings.get("subscription_status")
    if status_ not in ("canceled", "unpaid"):
        return False
    canceled_at = org_settings.get("subscription_canceled_at")
    if not canceled_at:
        return False  # lenient: no timestamp recorded yet, don't lock out
    try:
        since = datetime.fromisoformat(canceled_at.replace("Z", "+00:00"))
    except (ValueError, AttributeError):
        return False
    age_days = (datetime.now(timezone.utc) - since).total_seconds() / 86400
    return age_days > settings.billing_grace_days
...
def _resolve_user(token: str) -> dict:
    ...
    if cached and cached[1] > time.time():
        ...
        profile = (
            ...
            .select("org_id, role, must_change_password, organizations(settings)")
            .eq("id", user.id)
            .is_("deleted_at", "null")
            .single()
            .execute()
    ...
    org = profile_data.get("organizations") or {}
    return {
        "id": user.id,
        "email": user.email,
        ...
        "billing_suspended": _is_org_suspended(org.get("settings") if isinstance(org, dict) else None),
        "token": token,
    }
...
# Endpoints reachable while the org's subscription is suspended (lapsed past grace).
# The customer must be able to reach the billing portal to pay and restore access.
_BILLING_SUSPENDED_EXEMPT = ("/billing/portal",)
...
async def get_current_user(
    ...
    if credentials:
        ...
        # Lock out a lapsed paying org (past the grace period), leaving only the billing
        # portal reachable so they can pay and restore access.
        if user.get("billing_suspended") and not request.url.path.endswith(_BILLING_SUSPENDED_EXEMPT):
            raise HTTPException(
