...
class Settings(BaseSettings):
    ...
    # 0.0 = errors only, no performance tracing (matches this app's single-instance scale;
    # raise it if you need latency traces across the pipeline).
    sentry_traces_sample_rate: float = 0.0

    # ── Provisioning / billing ───────────────────────────────────────────────
    # Public URL of the app, used in the welcome email's login link. NO localhost in prod.
    app_base_url: str = "https://app.horusagents.com"
    # Comma-separated emails allowed into the cross-org super-admin panel (org creation).
    # Empty = the panel is disabled for everyone.
    superadmin_emails: str = ""
    # Self-service onboarding: any signed-up user creates their own org as admin.
    # OFF for the paid SaaS — orgs are provisioned by Horus (admin panel or payment).
    # Turn ON only for an open-source, single-tenant self-hosted deployment.
    allow_self_onboarding: bool = False
    # Stripe webhook signing secret (whsec_…). Set to enable /api/billing/webhook,
    # which auto-provisions an org when a customer completes Checkout.
    stripe_webhook_secret: str = ""
    # Stripe API secret key (sk_…). Needed to read subscription seat counts and to open
    # the Customer Portal (self-service seat changes / cancel / invoices).
    stripe_secret_key: str = ""
    # Stripe billing portal configuration id (bpc_…) created via the Stripe API. Optional:
    # if empty, the account's default portal configuration is used.
    stripe_portal_config_id: str = ""
    # Grace period (days) after a subscription becomes canceled/unpaid before the org is
    # locked out. During Stripe's own dunning (past_due) access stays normal; this grace is
    # ON TOP of that, measured from when the subscription finally went canceled/unpaid.
